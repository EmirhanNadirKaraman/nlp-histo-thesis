...
Asserts ``KnowledgeExtractionRunner._persist_voter_outputs`` constructs the
...
def _make_runner(records: list[dict]) -> tuple[KnowledgeExtractionRunner, MagicMock]:
    """Build a bare runner stub wired to a mock DB session.

    ``KnowledgeExtractionRunner.__init__`` requires LLMs that we don't want to
    ...
    """
    runner = object.__new__(KnowledgeExtractionRunner)
