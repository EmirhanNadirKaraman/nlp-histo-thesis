...
`SummarizationRunner.load_paper_from_db` (summarisation) used to call
...
from pipeline.stages.summarization import umls_resources as ur_mod
...
def test_load_paper_from_db_routes_through_singleton():
    """B-038 regression guard: `SummarizationRunner.load_paper_from_db`
    must NOT call `spacy.load` directly."""
    import inspect

    from pipeline.stages.summarization.runner import SummarizationRunner
    src = inspect.getsource(SummarizationRunner.load_paper_from_db)
    assert "get_small_nlp" in src, (
