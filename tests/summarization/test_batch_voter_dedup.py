"""Tests for in-run voter dedup in the batch summarization runner.
...
initialised ``BatchSummarizationRunner``; they don't hit any provider API.
"""
...
def _runner_for_dedup(l1, l2, l3) -> BatchSummarizationRunner:
    ...
    """
    r = object.__new__(BatchSummarizationRunner)
    r._l1 = l1
    r._l2 = l2
    r._l3 = l3
    return r
...
def test_voter_sig_treats_close_temps_as_equal():
    ...
    assert BatchSummarizationRunner._voter_sig(a) == BatchSummarizationRunner._voter_sig(b)
...
def test_voter_sig_separates_different_models_or_providers_or_temps():
    ...
    sigs = {BatchSummarizationRunner._voter_sig(v)
