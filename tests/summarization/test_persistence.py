"""Tests for the summarization filesystem-persistence layer.
...
def test_existing_summarization_imports_still_work():
    mod = importlib.import_module("pipeline.stages.summarization")
    for name in (
        "SummarizationRunner", "SummarizationConfig",
...
def test_runner_exposes_artifact_root_and_does_not_create_dirs_when_none(tmp_path: Path):
    ...
    import inspect
    sig = inspect.signature(SummarizationRunner.__init__)
