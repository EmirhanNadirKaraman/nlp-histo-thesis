...
from pipeline.stages.summarization.costing import (
    LLMUsageRecord, PriceBook, UsageCollector,
    load_records, write_cost_reports,
)
from pipeline.stages.summarization.costing.models import normalize_token_usage
from pipeline.stages.summarization.costing.report import build_report_from_records
from pipeline.stages.summarization.costing.pricing import ModelPrice
...
def test_missing_price_returns_none():
    ...
    lg = logging.getLogger("pipeline.stages.summarization.costing.pricing")
    lg.addHandler(h)
    try:
...
def test_skip_enrichment_env_no_ops(monkeypatch):
    ...
    from pipeline.stages.summarization import umls_resources
    from pipeline.stages.summarization.helpers.entity_linker import (
...
def test_config_disable_round_trips():
    ...
    from pipeline.stages.summarization.config import CostConfig, SummarizationConfig
    cfg = SummarizationConfig(cost=CostConfig(enable_cost_report=False))
