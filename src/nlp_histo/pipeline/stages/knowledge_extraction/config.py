"""
KnowledgeExtractionConfig — single source of truth for all numeric/boolean knobs
in the knowledge_extraction pipeline.

Usage
-----
from nlp_histo.pipeline.stages.knowledge_extraction.config import KnowledgeExtractionConfig, MapConfig, ResolveConfig
from dataclasses import replace

cfg = KnowledgeExtractionConfig()                         # all defaults
...
class AgreementConfig:
    ...
    * ``"greedy"`` / ``"hungarian"`` — one-to-one matching (each finding matched
    ...
    (``knowledge_extraction.agreement.hybrid.w_*``) and so swapping
...
class KnowledgeExtractionConfig:
    """
    All numeric/boolean knobs for KnowledgeExtractionRunner in one place.

    Operational params (output_dir, trace_enabled, db, LLM instances) stay
    on KnowledgeExtractionRunner itself since they cannot be expressed as plain data.
    """
