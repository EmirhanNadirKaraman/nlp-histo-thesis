...
# (B-066: the prior Path(__file__).parent pointed inside stages/ where no file
# exists. With the fallback removed, a missing or unreadable YAML now yields an
# empty synonym map and a loud error instead of silently masking the problem with
# a stale hardcoded dict.)
_SYNONYMS_RESOURCE = resources.files(
    "nlp_histo.pipeline.stages.knowledge_extraction.entities"
).joinpath("synonyms.yaml")
...
def _load_synonyms() -> dict[str, str]:
    ...
    try:
        ...
        data = yaml.safe_load(_SYNONYMS_RESOURCE.read_text(encoding="utf-8"))
    except FileNotFoundError:
        logger.error(
            "NORMALIZE: synonyms.yaml not found at %s — no synonyms loaded",
            _SYNONYMS_RESOURCE,
        )
        return {}
    ...
    logger.debug("NORMALIZE: loaded %d synonyms from %s", len(loaded), _SYNONYMS_RESOURCE)
    return loaded
