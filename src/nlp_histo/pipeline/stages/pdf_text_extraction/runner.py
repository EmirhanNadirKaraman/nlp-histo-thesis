...
Usage::
    ...
    from nlp_histo.pipeline.stages.pdf_text_extraction import PipelineConfig, PipelineRunner
...
class PipelineRunner:
    ...
    def _get_table_detector(self):
        if self._table_detector is None:
            ...
            if dtype == TableDetectorType.DOCLING:
                from nlp_histo.pipeline.stages.pdf_text_extraction.table_detectors import DoclingTableDetector
                self._table_detector = DoclingTableDetector()
            elif dtype == TableDetectorType.TATR:
                from nlp_histo.pipeline.stages.pdf_text_extraction.table_detectors import TATRTableDetector
                self._table_detector = TATRTableDetector(self._cfg.tatr)
            else:  # HYBRID or VLM (fallback to hybrid)
                from nlp_histo.pipeline.stages.pdf_text_extraction.table_detectors import HybridTableDetector
    ...
    def _get_two_pass_extractor(self):
        if self._two_pass_extractor is None:
            from nlp_histo.pipeline.stages.pdf_text_extraction.components.two_pass_extractor import TwoPassTextExtractor
    ...
    def _get_nlp(self):
        ...
        if self._nlp is None and needs_nlp:
            ...
            try:
                from nlp_histo.pipeline.stages.knowledge_extraction.entities.umls_resources import get_small_nlp
    ...
    def _steps_1_3_4_standard(
        ...
        """
        from nlp_histo.pipeline.stages.pdf_text_extraction.components.table_reconstructor import reconstruct_tables_from_lists
    ...
    def _steps_1_3_4_two_pass(self, pdf_path: Path, pmcid: str):
        ...
        """
        from nlp_histo.pipeline.stages.pdf_text_extraction.models.dto import LayoutResult as _LR
        from nlp_histo.pipeline.stages.pdf_text_extraction.components.table_reconstructor import reconstruct_tables_from_lists
