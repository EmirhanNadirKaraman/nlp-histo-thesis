...
# (or empty) replay — so the root is an explicit, required argument.
#
# Importing this module now creates no directories and reads no .env.
_REPO_ROOT: Path = None  # type: ignore[assignment]
...
def configure(artifact_root: Path, output_dir: Path | None = None) -> None:
    ...
    OUT_DIR = (
        ...
        else root / DEFAULT_OUTPUT_SUBPATH
    )
    ...
    # The matcher's embedder constructor requires a non-empty API key even when the
    # embedding cache is warm and no call is issued. Offline still, no spend.
    try:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Directory containing out/ and eval/ (a repository checkout, or a copy "
             "of the frozen thesis artifacts with the same layout).",
    )
    parser.add_argument(
