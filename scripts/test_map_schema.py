...
def test_direction_enum_has_no_direction():
    from pipeline.stages.summarization.models import DirectionEnum
...
def test_relation_type_enum():
    from pipeline.stages.summarization.models import RelationTypeEnum
...
def test_prompt_lists_match_enums():
    from pipeline.stages.summarization import prompts
...
def test_invalid_relation_type_coerces_to_unclear():
    from pipeline.stages.summarization.models import Finding, RelationTypeEnum
    f = Finding.model_validate({
        "category": "morphology",
        "claim": "X has Y",
        "evidence": ["S1|PMC1|1"],
...
def test_cascade_signature_stable_and_sensitive():
    from pipeline.stages.summarization.models import compute_cascade_signature
...
def test_batch_handle_persists_versions():
    import tempfile
    from pathlib import Path as _P
    from pipeline.stages.summarization.batch.models import BatchHandle, BatchPhase
