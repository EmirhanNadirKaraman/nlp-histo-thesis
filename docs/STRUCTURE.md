...
| 2026-05-26 | Summarisation › `agreement.scorer_kind` promoted to config | `AgreementConfig` gains `scorer_kind: str = "embedding"` (Literal `"embedding" \| "hybrid"`). New `SemanticAgreementScorer.from_agreement_config(agreement_cfg, embed_fn=…)` classmethod dispatches to `EmbeddingSimilarityStrategy.from_config` or `HybridStructuredSimilarity.from_config` based on `scorer_kind`; raises `ValueError` on any other value. Three production callsites switched from the hardcoded `EmbeddingSimilarityStrategy.from_config(...)` build to the classmethod: `runner.py:212–215`, `batch/runner.py:158–161` (ctor), `batch/runner.py:1147–1150` (per-level rebuild). Stage 1 of the MAP sweep (`run_summarization_sweeps --stage map_scorer`) is now production-pinnable in YAML: the pin hint now reads `→ pin: BEST_SCORER = '…'  (update configs/run.yaml summarization.agreement.scorer_kind: …)`. Production default `"embedding"` preserves the historical behaviour exactly. Hybrid blend weights (`w_category` / `w_embedding` / `w_entity` / `w_evidence`) remain at `HybridStructuredSimilarity` constructor defaults — deferred to a follow-up promotion, only relevant if Stage 1 picks hybrid. Existing voter_cache + embedding_cache sufficient (post-cache decision); no API spend. | THESIS.md Decisions log 2026-05-26 |
| 2026-05-26 | Summarisation › `agreement.force_escalate_on_polarity_conflict` promoted to config | `AgreementConfig` gains `force_escalate_on_polarity_conflict: bool = True` — the B-051 hard-fail policy is now a configurable flag rather than unconditional. `AgreementChecker.compute` restructured: when a polarity conflict is detected, the marker (`score_details["hard_fail_reason"] = "polarity_conflict"` + `polarity_conflict_details.count`) is **always recorded**; the decision override to `ChunkDecision.ESCALATE` is now gated on the flag. False = scorer's natural decision stands; True (default) = unchanged B-051 behaviour. All 12 existing B-051 tests continue to pass (they operate at the default); 3 new tests in `test_b051_hard_fail_polarity.py` exercise the False branch using the high-confidence scorer stub. Five AgreementChecker callsites threaded the new kwarg (`runner.py`, `batch/runner.py` ×2, `map_stage.py`, `map_theta_sweep.py::_replay`). Sweep harnesses extended: `map_theta_sweep.py --force-escalate-on-polarity-conflict {true,false,both}` (default `true` preserves historical CSV output); new `--stage map_polarity_flag` in `run_summarization_sweeps.py` for the 2-cell B-051 ablation. Two new CSV columns on both harnesses: `n_polarity_conflict_chunks` (absolute) + `polarity_conflict_rate` (rate over total chunks) — independent of flag value so True vs False rows can be diffed. No API spend; pure offline replay over the existing primer. Hash-included; flipping invalidates the per-paper summary cache. | THESIS.md Decisions log 2026-05-26 |
| 2026-05-26 | Summarisation › `agreement.hybrid` (HybridStructuredSimilarity blend weights) promoted to config | New `HybridConfig` dataclass with `w_category`/`w_embedding`/`w_entity`/`w_evidence` (defaults `0.25 / 0.40 / 0.25 / 0.10`, Σ=1.0); added as a nested field `hybrid: HybridConfig` on `AgreementConfig`. `HybridStructuredSimilarity.from_config` now reads `agreement_cfg.hybrid.w_*` and merges with the existing `**signal_weights` back-compat path (caller-supplied kwargs win per-field). `EmbeddingSimilarityStrategy` is unaffected — the hybrid block is silently ignored on the embedding scorer path. New Stage 1b `map_hybrid_blend` in `run_summarization_sweeps.py`: 6 labelled, sum-to-1 weight variants (`default`, `balanced`, `embedding_heavy`, `category_heavy`, `entity_heavy`, `evidence_heavy`) at the pinned BEST_THETA/BEST_REJECT_THETA. Gated on `BEST_SCORER == "hybrid"` — mirrors the existing Stage 3 `map_weights` conditional. Sweep CSV gains 5 columns: `w_category` / `w_embedding` / `w_entity` / `w_evidence` / `weights_sum` (always emitted regardless of scorer — `weights_sum` is the audit trail for the Σ=1 convention, which is documented but not enforced). Existing voter_cache + embedding_cache sufficient for replay across all 6 variants; no API spend. Hash-included via `dataclasses.asdict(cfg)`; flipping invalidates the per-paper summary cache. | THESIS.md Decisions log 2026-05-26 |
| 2026-05-26 | Eval › top-level experiment orchestrator `scripts/eval/run_summarization_experiments.py` | New top-level orchestrator (~570 LOC) mirroring `scripts/eval/run_all_sweeps.py`'s style. Defines 13 `ExperimentSpec`s for the FINAL doc's MAP-stage recipe — 7 calibration EXPs (EXP_1–EXP_7, Gemini branch → OpenAI branch → embedder comparison), 1 added held-out test confirmation (EXP_F), and 5 validation EXPs (EXP_A–EXP_E, shipped as recipe stubs that point at the FINAL doc). CLI mirrors `run_all_sweeps.py`: `--stage` / `--branch` / `--exp` / `--only` / `--list-variants` / `--run`. Topological sort respects per-EXP `depends_on`; BEST_* state flows from one EXP to the next via `ExperimentResult.state_updates`. Default is dry-run; `--run` is opt-in. The orchestrator does not duplicate sweep mechanics — calibration EXPs call `run_sweep` from `eval/silver/map_theta_sweep.py` and `_load_map_context` from `run_summarization_sweeps.py` directly (no subprocess). Per-scorer best-of selection (EXP 1 / EXP 4) fixes the FINAL doc's "fair scorer comparison" rule — each scorer config gets its own optimal θ/reject_θ before scorer-to-scorer comparison. | THESIS.md Decisions log 2026-05-26, [`docs/FINAL_SUMMARIZATION_EXPERIMENTS.md`](FINAL_SUMMARIZATION_EXPERIMENTS.md) |
| 2026-05-26 | Eval › embedding-cache JSON retired; SQLite is the only on-disk format | The Gemini JSON embedding cache (`embedding_cache_gemini.json`, 720 MB / 17,827 entries) was retired now that the sibling SQLite (`embedding_cache_gemini.sqlite`, 250 MB / 20,317 entries) is a strict superset. Constants `DEFAULT_CACHE_PATH` and `DEFAULT_GEMINI_CACHE_PATH` in `eval/silver/matcher.py` renamed `.json → .sqlite` so the path matches what's actually on disk (the sqlite-backend path-suffix rewrite in `make_embedding_cache` was hiding the inconsistency). `scripts/import_embedding_cache_sqlite.py` no longer defaults `--json` to the now-missing in-repo file; the script is retained for external imports with explicit `--json`. JSON backend code path (`EmbeddingCache` class + `NLP_HISTO_EMBEDDING_CACHE_BACKEND=json`) left intact for tests and ad-hoc imports. Docs updated: `HOW_TO_RUN.md`, `CALIBRATION_EXECUTION_PLAN.md`, `SUMMARIZATION_EXPERIMENT_PLAN.md`. `BUGS.md` B-064 historical entry untouched per project rules. No behaviour change at production paths — both before and after the rename, `make_embedding_cache(…)` returned the same `SQLiteEmbeddingCache(…sqlite)` instance. | THESIS.md Decisions log 2026-05-26 |
| 2026-05-27 | Eval › EXP B.2 promoted from stub to executable; new `--phase baselines` | `scripts/eval/run_summarization_experiments.py::_run_exp_b2` replaces its prior stub recipe with a full implementation. New `baselines` phase in `PHASE_ORDER`/`PHASE_TO_EXPS`/`PHASE_BLURBS` runs only EXP B.2 (`PHASE_TO_EXPS["baselines"] = ("EXP_B.2",)`). CSV layout: per-L1-voter rows, an `l1_best_oracle` row (per-case silver-selected, clearly labelled oracle/not-deployable), per-L2-voter rows (mid-tier strong-model baseline; sizes the L1→L2 vs L2→L3 accuracy gaps), an `l3_only` (Sonnet) row, plus a `cascade_provisional_final` row when `PROVISIONAL_FINAL_MAP_CONFIG` is in state. Columns: split, baseline, model_or_source, strict_f1, f1, precision, recall, n_matched, n_silver, n_pipeline, estimated_cost_per_chunk, notes. Cost-per-chunk derived from `configs/model_prices.json` at a fixed (1500 input + 600 output) token assumption (constants `_B2_TOK_IN`, `_B2_TOK_OUT`, table `_B2_PER_VOTER_COST` — recompute if the price book moves). Replay-only: reads `eval/data/map_primer/voter_cache.json` (which has L2 and L3 populated for every chunk — 303/303 verified for both), no API calls. Cascade row builds via `map_theta_sweep._replay` + `_load_map_context`; warm `embedding_cache_gemini.sqlite` covers it for free. **No state mutation** by design: `ExperimentResult.state_updates = {}` so `PROVISIONAL_FINAL_MAP_CONFIG` and the BEST_/FINAL_ keys are never overwritten by a diagnostic phase. Orchestrator now calls `load_dotenv(.env)` at import (matcher embedder is OpenAI text-embedding-3-small, needs `OPENAI_API_KEY` for any cache miss; EXPs 1-7 got this transitively via `run_summarization_sweeps.py:load_dotenv`, B.2 does not import that module). New module constants `_B2_CACHE_PATH` and `_B2_SILVER_PATH` are monkey-patchable for tests. Five new tests in `tests/eval/test_run_summarization_experiments.py` cover phase routing, missing-cache hard-fail, L3-only completeness, L1 per-voter row construction, and the no-state-mutation contract. The 2026-05-26 row claiming "EXP B.2 needs a separate Sonnet primer" is now factually wrong; that row stays for history and the corrective rationale lives in the THESIS Decisions log entry for 2026-05-27. | [EXP_B2_RESULTS.md §1](EXP_B2_RESULTS.md), THESIS.md Decisions log 2026-05-27, [`docs/FINAL_SUMMARIZATION_EXPERIMENTS.md` §EXP B.2](FINAL_SUMMARIZATION_EXPERIMENTS.md) |
| 2026-05-27 | Eval › `scripts/eval/b2_family_bias_diagnostic.py` (family-bias methodological check for EXP B.2) | New standalone script (~180 LOC) under `scripts/eval/`. Reads `voter_cache.json` + the OpenAI matcher embedding cache, computes two stats per voter — `mean_matched_sim` (avg cosine over matched ≥0.55 pairs) and `mean_best_per_silver_sim` (avg over silver findings of the max cosine to any of that voter's findings, threshold-free) — and aggregates by family. Replay-only, no API calls (cache warm from EXP B.2). Outputs `eval/reports/exp_b2_family_bias_diagnostic_<ts>.csv` + a console table. Reuses `_b2_build_voter_outputs` from `run_summarization_experiments.py` (loaded via `importlib.util.spec_from_file_location` since `scripts/` isn't a package). Voter-spec table `VOTER_SPECS` enumerates the 7 production voters with (level, index, model, family) tuples. Result on the dev-split run: Anthropic−OpenAI Δ = +0.027 (matched) / +0.048 (best-per-silver), both **below the 0.055 evidence threshold** (10 % of sim_threshold). gpt-4.1-mini's matched-pair similarity (0.895) is **higher** than Haiku's (0.889) — the matcher does not favour Anthropic phrasing inside the matched zone. Conclusion: the EXP B.2 cost-effectiveness ranking is not a family-bias artifact. | [EXP_B2_RESULTS.md §2](EXP_B2_RESULTS.md), THESIS.md Decisions log 2026-05-27 |
