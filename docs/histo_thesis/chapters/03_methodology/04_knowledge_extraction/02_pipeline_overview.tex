...
The pipeline consists of seven stages, where each stage consumes the typed object the previous stage produced. The flow is mostly linear, except at the end: RELATE stage computes the pairwise relations of the canonical rules, and RESOLVE scores these rules using the relations. RESOLVE, therefore, is the only stage that consumes two inputs: the canonical rules from CANONICALIZE and the relations from RELATE. The seven stages in the pipeline are as shown in Figure~\ref{fig:ke-pipeline}, with the input and output types per stage.
