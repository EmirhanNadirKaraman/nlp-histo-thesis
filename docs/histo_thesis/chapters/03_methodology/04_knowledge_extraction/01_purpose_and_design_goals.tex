...
\begin{itemize}[label={}, leftmargin=1.5em, itemsep=0.4em]
    ...
    \item \textbf{Bounded LLM stochasticity.} API calls to generative large language models are confined to the MAP stage described in Subsection~\ref{subsec:knowledge-map}; every later stage is deterministic. This design concentrates the pipeline's stochastic behavior into a single controlled stage, rather than spreading it across the full pipeline.
