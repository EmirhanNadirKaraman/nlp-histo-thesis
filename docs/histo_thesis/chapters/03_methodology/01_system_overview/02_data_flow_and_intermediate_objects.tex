...
\begin{itemize}[label={}, leftmargin=1.5em, itemsep=0.4em]
    ...
    \item \textbf{Text elements.} Paragraphs and headings, each with its unique path, content, depth, and position within the section. A text element points back to its document and is the unit consumed by the knowledge extraction pipeline.
    
    \item \textbf{Figures and tables.} Cropped images of figures and tables identified in papers. Each figure and table includes its caption and bounding box information, as well as cross-references when it is mentioned.
    ...
    \item \textbf{Relations.} Pairwise edges between canonical rules, each labeled \texttt{SUPPORT}, \texttt{CONTRADICT}, or \texttt{UNRELATED} by an NLI cross-encoder. Only the supporting and contradicting pairs are kept. Each relation has the two canonical rules it connects, alongside its \texttt{SUPPORT}/\texttt{CONTRADICT} label and the directional NLI scores.

    \item \textbf{Final rules.} Canonical rules are augmented with scores to generate final rules. A canonical rule is given a score between 0 and 1, combining its grounding score, the number of findings behind it, and the support and contradiction relations that touch it. The generated final rule keeps a pointer back to its canonical rule.
\end{itemize}
