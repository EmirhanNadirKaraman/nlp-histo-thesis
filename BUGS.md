...
> `.claude/` — all **local-only and gitignored**. Those paths are written as
> inline code rather than as Markdown links, because a link that only resolves
> on the maintainer's machine looks navigable to a reader who clones the
> repository and is not. The write-up itself carries the evidence. Separately,
> historical entries quote the file layout **as it was when the bug was found**
> — many predate the `src/nlp_histo/` packaging migration. Those paths are
> evidence, not drift: do not "correct" them.
...
* `pipeline/stages/pdf_text_extraction/__init__.py` — dropped the
  ...
  from `__all__`.
* `readmes/other_readmes/REPOSITORY_GUIDE.md` — removed the
...
* Cataloged as Issue 5 in `readmes/other_readmes/MAP_PROMPT_AUDIT.md#issue-5--scopescope_parsed-is-llm-set-but-trivially-derivable-low`.
...
* `pipeline/stages/summarization/models.py`
  ...
  * Added `POLARITY_BEARING_DIRS = frozenset({"positive", "negative", "absent", "partial"})` and `NON_POLARITY_DIRS = frozenset({"unclear", "no_direction"})` as the single source of truth used by canonicalize / relate / corpus_relate. `partial` is included in the polarity set so `positive + partial → is_conflicted=True`; this is not a final decision on partial's semantics — owned by [B-025](#bug-25--relate-polarity-guard-treats-partial-as-positive-blocking-partial-vs-positive-contradictions).
...
* **`partial` semantics** — B-049 keeps `partial` in `POLARITY_BEARING_DIRS` for the group-level flag and in `_POSITIVE_DIRECTIONS` for the CONTRADICT gate. Whether either is right is owned by [B-025](#bug-25--relate-polarity-guard-treats-partial-as-positive-blocking-partial-vs-positive-contradictions) and needs a calibration sweep before changing.
...
**Status / Severity / Surface** — Fixed (2026-07-18, `99f8aaf`) · Medium · Reproducibility.
`eval/silver/analysis/map_theta_sweep.py` (`PRIMER_DIR`) × `scripts/thesis/run_chapter9_offline_replay.py`.
...
`eval/silver/analysis/map_theta_sweep.py`, the `PRIMER_DIR` constant:
...
Fixed in `99f8aaf` by anchoring the repository-owned defaults to
`eval.paths.REPO_ROOT` — the same resolver `_FROZEN_OPENAI_CACHE` /
`_FROZEN_GEMINI_CACHE` four lines above already used, so the module had been half
anchored since those caches were added:

1. `PRIMER_DIR`, with `PRIMER_PATH` / `CACHE_PATH` still derived from it. These two
   are default argument values on eight functions and bind at import time, which is
   why a single relative constant reached every call site.
2. `SOURCE_PATH` / `SILVER_PATH`, frozen inputs carrying the same trap. Both are
   currently unreferenced, so this changes no behaviour today and removes a latent
   one.

Point 3 of the original plan — whether repo-root-cwd is a supported contract or a bug —
was decided as *a bug for inputs, a contract for outputs*. `REPORTS_DIR` is deliberately
left caller-relative and commented as such: it is generated output, `--reports` defaults
to it, and writing results next to wherever the command ran is the intended behaviour.
Explicit `--primer-dir` / `--source` / `--silver` / `--embed-cache` overrides keep their
caller-relative semantics.

### Verification

`tests/test_map_theta_sweep_paths.py` — 13 tests that import the module **with cwd set to
a temporary directory outside the repository**, the condition that used to break. They
assert the defaults are absolute, under the repository root, identical when imported from
two different cwds, and equal to what the old relative literals named from the repo root;
one drives the public `--help` from a foreign cwd. Full suite 1710 passed.

Results equivalence, since this touches a path the thesis numbers are read through:
E14 `--theta-frontier` was replayed at the parent commit and at the fix, `strict_cache_only`,
under a `getaddrinfo` guard that raises on provider hosts — **zero billable attempts on both
runs**, and the two CSVs are byte-identical (sha256 `848cdb72…`), matching the 0.7128 that
THESIS.md pins for E14 at θ0.90. The four frozen inputs were checksummed before and after:
`primer.json` `6d6264a3`, `voter_cache.json` `365047b0`, and both embedding caches unchanged.

Demonstration of the fix from `/private/tmp`: the parent constant resolves to
`/private/tmp/eval/data/map_primer/voter_cache.json` (does not exist — the bug); the anchored
constant resolves to the real file under the repository root.
...
**Status / Severity / Surface** — Fixed (2026-07-18, `6256807`) · Medium · Eval reproducibility.
`eval/sweeps/grounding.py` (`DEFAULT_OUT_MD` / `DEFAULT_OUT_CSV`) × `eval/results/grounding_sweep.md` (tracked).
...
Of the three options, **default the output to an untracked path**. Pinning the input set
would make the command reproduce a 5-paper snapshot rather than report the current corpus,
which is not what a sweep is for; untracking the `.md` would remove a published artifact the
thesis cites. Redirecting the writer keeps both properties.

`DEFAULT_OUT_CSV` / `DEFAULT_OUT_MD` now point at `out/eval_sweeps/`, which `.gitignore`
covers via `out/*`. The tracked snapshot stays tracked and untouched; `--out-md` still
reaches it, so regenerating it remains possible but must be asked for.

That the sweep pins no input is *not* fixed and is not a defect: it reports the corpus it is
given. The defect was writing that report over a frozen artifact.

### Verification

`tests/eval/sweeps/test_grounding_output_defaults.py` — 8 tests asserting the defaults are
not tracked paths (checked against `git ls-tree HEAD`, not the working directory), that they
live under `out/`, that the snapshot is still tracked, and that an explicit `--out-md`
override still reaches it.

Empirically: `python eval/sweeps/grounding.py` run verbatim over the current 15-paper
`out/summaries` — the tracked report's sha256 is `52dd25db…` before and after, `git status`
reports no modification to `eval/results/`, and the output lands in `out/eval_sweeps/`. The
43 pre-existing sweep tests still pass, including the `conftest.py` guard that already
asserted no test may mutate the tracked report.
...
than issuing a paid call — the same guarantee as [B-112](#bug-112--the-replays-paid-call-guard-misses-the-gemini-embedding-cache).
...
constructor theatre, the same defect as [B-109](#bug-109--free-cached-experiments-hard-require-an-unused-googleapikey)
and [B-112](#bug-112--the-replays-paid-call-guard-misses-the-gemini-embedding-cache).
